       ...
       BlockState state)
{
   ...
   // [BG]
   // If the block is being brought to the cache now for the first time,
   // A block is moved at position that is 3 elements before the tail for
   // FLRU policy
   else if (replacement_policy == ReplacementFLRU && block->tag != tag){
       set->lru_list.Erase(block->lru_node);
       misc::List<mem::Cache::Block>::Iterator insert_position= set->lru_list.end();
       --insert_position;
       --insert_position;
       --insert_position;
       set->lru_list.Insert(insert_position, block->lru_node);
   }
   ...
   // [BG]
   // set the frquency of accessed as 1
   block->frequency = 1;
}
...
void Cache::AccessBlock(unsigned set_id, unsigned way_id)
{
   ...
   // [BG]
   else if (replacement_policy == ReplacementFLRU){
       set->lru_list.Erase(block->lru_node);
       set->lru_list.PushFront(block->lru_node);
   }
...
unsigned Cache::ReplaceBlock(unsigned set_id)
{
   ...
   // [BG]
       ...
       // Get the three blocks at the end of LRU list
       // Replace the one that has the lowest access frequency
       // if there are 2 lowest, replace the last block
       Block* block;
       Block *block_last = misc::cast<Block *>(set->lru_list.Back());
       assert(block_last);
       unsigned lowest_frequency = block_last->frequency;
       block = block_last;
       Block *block_second_last = misc::cast<Block *>(set->lru_list.previous(block_last->lru_node));
      
       if (block_second_last){


           if (block_second_last->frequency < lowest_frequency ){
               block = block_second_last;
               lowest_frequency = block->frequency;
           }
          
           Block *block_third_last = misc::cast<Block *>(set->lru_list.previous(block_second_last->lru_node));    
           if(block_third_last){
               if (block_third_last->frequency < lowest_frequency){
                   block = block_third_last;
                   lowest_frequency = block->frequency;
               }
           }
       }
